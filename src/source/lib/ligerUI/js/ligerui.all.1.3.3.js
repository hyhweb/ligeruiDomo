                ...
                if (value != null)
                {
                    ...
                    for (var i = 0; i < targetdata.length; i++)
                    {
                        $("table.l-table-checkbox tr[value=" + targetdata[i] + "] :checkbox", g.selectBox).each(function () {
                            this.checked = true;
                            $(this).siblings('.l-checkbox').addClass('l-checkbox-checked');
                        });
                ...
                success: function (result)
{
        ...
        _filter:function (pData,pVal) {
            var g = this, p = this.options;

            if (p.filter) {
                return p.filter(pData,pVal);
            }

            var vList=[];
            var vVal=pVal.toLowerCase();
            for (var i=0;i<pData.length;i++) {
                var val = pData[i][p.valueField].toLowerCase();
                var txt = pData[i][p.textField].toLowerCase();

                if (val.indexOf(vVal)>=0||txt.indexOf(vVal)>=0) {
                    vList.push(pData[i]);
                }
            }
            return vList;
            ...
            g.inputText.keyup(function (event)
            {
                ...
                { 
                    ...
                    } else if (p.grid)
                    {
                        ...
                        var vData=g._filter(p.data,currentKey);
                        g.setData(vData);
                        g._selectBoxShow();
                    }
